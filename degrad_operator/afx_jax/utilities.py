...
def apply_utility(
    input_signal, afx_type, gain_staging, sr, signal_len, mono, **param_dict
):
    ...
    if afx_type == "panning":
        ...
        h = get_signal(input_signal, "ir")
        y = convolve(x, h)
        if gain_staging:
            y = gain_stage(x, y)
        return {"main": y}
    elif afx_type == "adder":
        h = get_signal(input_signal, "noise")
        y = adder(x, h)
        if gain_staging:
            y = gain_stage(x, y)
        return {"main": y}
...
def convolve(x, h, mode="full"):
    ...
    out = jax.scipy.signal.convolve(x, h, mode=mode, method='fft') # "full", "same"
...
def adder(x, h):
    ...
    if len(h) > length:
        h = h[:length]
    h_padded = jnp.pad(h, (0, length - len(h)), mode='constant')
