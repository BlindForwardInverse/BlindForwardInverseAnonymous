...
from diffusion_sampler.sde_class_hybrid_proj import ProbODEFlowHybridProj, sigma_scheduler
from diffusion_sampler.sde_solver_hybrid_proj import Heun2ndSamplerHybridProj
...
class DiffusionHybridSolverProj(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.projector = UnetWrapperHybridWithWeight(unet_channels = 32,
                                               cond_dim = cond_dim,
        ...
        # SDE
        self.sde = ProbODEFlowHybridProj(f_theta = self.model, 
                                   ...
                                   **kwargs)
        self.sampler = Heun2ndSamplerHybridProj(self.sde,
                                      diffusion_steps=diffusion_steps,
                                      **kwargs)
        self.sigma_list = sigma_scheduler(N=256, 
                                          rho=train_rho,
                                          sigma_min=sigma_min,
    ...
    def forward(self, dry_spec, wet_spec, dry_wav, wet_wav, global_condition, local_condition):
        ...
        -----------------------------------
        '''
#         log_sigma = torch.randn(batch_size, device=device) * 1.2 - 3.6
#         sigma = torch.clamp(torch.exp(log_sigma), min=self.sigma_min, max=self.sigma_max)
    ...
    def projection(self, denoised, wet_spec, wet_wav, global_condition, local_condition):
        denoised_spec = self.wav_to_spec(denoised).detach()
        spec_input = denoised_spec
        wav_input = denoised
        spec_input = torch.cat([denoised_spec, wet_spec], dim=-1)
        wav_input = torch.stack([denoised, wet_wav], dim=1)
    ...
    def training_step(self, batch, _):
        ...
        model_output_spec, model_output_summed, target_spec, target_summed, denoised = \
            self(dry_spec, wet_spec, dry_wav, wet_wav, global_condition, local_condition)
        ...
        if self.use_discriminator:
            ...
            return diffusion_loss + projection_loss
...
#         self.log_dict(estoi)
    ...
    def collect_model_params(self):
        ...
        model_params += list(self.projector.parameters())
        return model_params
