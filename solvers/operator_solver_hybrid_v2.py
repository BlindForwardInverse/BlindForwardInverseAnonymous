...
class OperatorSolverHybridV2(pl.LightningModule):
    def __init__(
        self, save_dir=None, clip_grad=False, save_audio_per_n_epochs=5,
        # Signal Spec
        ...
        mse_weight=10., factor_sc=1, factor_mag=1, factor_phase=0, pred_loss_weight=0.1,
        # Model
        ...
        **kwargs,
        ...
        self.separate_discriminator = separate_discriminator
        ...
        if use_discriminator:
            if separate_discriminator:
                self.discriminator_spec = MultiResolutionDiscriminator()
                self.discriminator_wav = MultiResolutionDiscriminator()
            else:
    ...
    def forward(self, dry_tar_spec, dry_tar_wav, wet_ref, wet_ref_spec=None):
        ...
        if self.learnable_sum:
            ...
            summed_wav = rearrange(weight[:, 0], 'b -> b 1') * pred_wav.detach() +\
                         rearrange(weight[:, 1], 'b -> b 1') * self.spec_to_wav(pred_spec).detach()
        else:
    ...
    def training_step(self, batch, _):
        ...
        out_spec_transformed = self.spec_to_wav(out_spec)

        # Loss Function
        pred_loss_spec, loss_dict_spec = self.prediction_loss(out_spec, wet_tar_spec, out_spec_transformed, wet_tar_wav, tag='_spec')
        pred_loss_wav, loss_dict_wav  = self.prediction_loss(out_wav, wet_tar_wav, out_wav, wet_tar_wav, tag='_wav')
        pred_loss_blend, loss_dict_blend = self.prediction_loss(pred_tar_wav, wet_tar_wav, pred_tar_wav, wet_tar_wav, tag='_blend')
        pred_loss = (pred_loss_spec + pred_loss_wav + pred_loss_blend) / 3.0
        
        self.log("pred_loss_spec", pred_loss_spec, prog_bar=True, batch_size=self.batch_size, on_epoch=True)
        self.log("pred_loss_wav", pred_loss_wav, prog_bar=True, batch_size=self.batch_size, on_epoch=True)
        self.log("pred_loss_blend", pred_loss_blend, prog_bar=True, batch_size=self.batch_size, on_epoch=True)
        self.log_dict(loss_dict_blend, prog_bar=True, batch_size=self.batch_size, on_epoch=True)
        ...
        if self.use_discriminator:
            ...
            # Discriminator Step
            if self.separate_discriminator:
                disc_real_logits_spec = self.discriminator_spec(wet_tar_wav)
                disc_real_logits_wav = self.discriminator_wav(wet_tar_wav)
                disc_fake_logits_spec = self.discriminator_spec(out_spec_transformed.detach())
                disc_fake_logits_wav = self.discriminator_wav(out_wav.detach())

                disc_loss_spec, _ = self.discriminator_loss(disc_real_logits_spec, disc_fake_logits_spec)
                disc_loss_wav, _ = self.discriminator_loss(disc_real_logits_wav, disc_fake_logits_wav)

            else:
                disc_real_logits = self.discriminator(wet_tar_wav)
                disc_fake_logits_spec = self.discriminator(out_spec_transformed.detach())
                disc_fake_logits_wav = self.discriminator(out_wav.detach())

                disc_loss_spec, _ = self.discriminator_loss(disc_real_logits, disc_fake_logits_spec)
                disc_loss_wav, _ = self.discriminator_loss(disc_real_logits, disc_fake_logits_wav)

            disc_loss = 0.5 * (disc_loss_spec + disc_loss_wav)
            ...
            if self.global_step > self.freeze_disc_steps:
                if self.separate_discriminator:
                    gen_fake_logits_spec = self.discriminator_spec(out_spec_transformed)
                    gen_fake_logits_wav = self.discriminator_wav(out_wav)
                else:
                    gen_fake_logits_spec = self.discriminator(out_spec_transformed)
                    gen_fake_logits_wav = self.discriminator(out_wav)
                generator_loss_spec = self.generator_loss(gen_fake_logits_spec)
                generator_loss_wav = self.generator_loss(gen_fake_logits_wav)
                generator_loss = 0.5 * (generator_loss_spec + generator_loss_wav)
    ...
    def configure_optimizers(self):
        ...
        if self.use_discriminator:
            if self.separate_discriminator:
                disc_param = list(self.discriminator_spec.parameters()) +\
                                list(self.discriminator_wav.parameters())
            else:
                disc_param = list(self.discriminator.parameters())

            disc_optim = torch.optim.AdamW(params=disc_param,
