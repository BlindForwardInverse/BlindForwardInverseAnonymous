...
from models.operator_models.unet import UnetWrapper1d
...
class OperatorSolver1d(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.unet = UnetWrapper1d(unet_channels=unet_channels,
                                    cond_dim=cond_dim,
                                    embedding_size=embedding_size,
                                    cat_ref=cat_ref,
                                    **kwargs)
    ...
    def forward(self, dry_tar, wet_ref):
        # Unet Input
        unet_input = torch.stack([dry_tar, wet_ref], dim=1) if self.cat_ref else dry_tar
        ...
        pred_tar = self.unet(unet_input, t=global_condition, condition=local_condition)
        return pred_tar

    def training_step(self, batch, _):
        # dry_tar, wet_tar = compressed spectrogram
        # wet_ref = waveform
        dry_tar, wet_tar, wet_ref = [batch[key] for key in ["dry_tar", "wet_tar", "wet_ref"]]
        pred_tar = self(dry_tar, wet_ref)

        wet_tar_wav = wet_tar
        pred_tar_wav = pred_tar

        pred_loss, pred_loss_dict = self.prediction_loss(pred_tar, wet_tar, pred_tar_wav, wet_tar_wav)
        ...
        if self.use_discriminator:
            ...
            if self.discriminator_type == 'MRD':
                ...
                disc_real_logits = self.discriminator(wet_tar)
                disc_fake_logits = self.discriminator(pred_tar.detach())
            elif self.discriminator_type == 'MRD+MPD':
                real_mrd, real_mpd = self.discriminator(wet_tar_wav)
                fake_mrd, fake_mpd = self.discriminator(pred_tar_wav.detach())

            # discriminator backward
            if self.discriminator_type == 'MRD+MPD':
            ...
            if self.global_step > self.freeze_disc_steps:
                if self.discriminator_type == 'MRD':
                    ...
                    gen_fake_logits = self.discriminator(pred_tar)
                    generator_loss = self.generator_loss(gen_fake_logits)
                elif self.discriminator_type == 'MRD+MPD':
    ...
    def validation_step(self, batch, idx):
        with self.ema.average_parameters():
            dry_tar_wav, wet_tar_wav, dry_ref_wav, wet_ref_wav = \
                [batch[key] for key in ["dry_tar", "wet_tar", "dry_ref", "wet_ref"]]
            valid_types, afx_names, afx_types, audio_nums = \
                [batch[key] for key in ["valid_type", "afx_name", "afx_type", "audio_idx"]]

            pred_tar_wav = self(dry_tar_wav, wet_ref_wav)
    ...
    def configure_optimizers(self):
        ...
        if self.use_discriminator:
            gan_optim = torch.optim.AdamW(params=self.discriminator.parameters(), lr=self.lr, weight_decay=0.01)
            return model_optim, gan_optim
        else:
