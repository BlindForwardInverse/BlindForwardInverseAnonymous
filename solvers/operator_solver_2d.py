...
from models.operator_models.unet import UnetWrapper
...
class OperatorSolver2d(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.unet = UnetWrapper(unet_channels=unet_channels,
                                unet_only=use_ref_encoder,
                                cond_dim=cond_dim,
                                embedding_size=embedding_size,
                                cat_ref=cat_ref,
                                **kwargs)
    ...
    def forward(self, dry_tar, wet_ref, wet_ref_spec=None):
        # Unet Input
        unet_input = torch.cat([dry_tar, wet_ref_spec], dim=-1) if self.cat_ref else dry_tar
    ...
    def training_step(self, batch, _):
        ...
        pred_tar_spec = self(dry_tar_spec, wet_ref_wav, wet_ref_spec)
        pred_tar_wav = self.spec_to_wav(pred_tar_spec)

        # Loss Function
        pred_loss, pred_loss_dict = self.prediction_loss(pred_tar_spec, wet_tar_spec, pred_tar_wav, wet_tar_wav)
    ...
    def validation_step(self, batch, idx):
        with self.ema.average_parameters():
            dry_tar_wav, wet_tar_wav, dry_tar_spec, wet_tar_spec, dry_ref_wav, wet_ref_wav, wet_ref_spec =\
                [batch.get(key) for key in ["dry_tar", "wet_tar", "dry_tar_spec", "wet_tar_spec", "dry_ref", "wet_ref",  "wet_ref_spec"]]
