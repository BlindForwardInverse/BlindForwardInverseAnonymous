...
from baseline.rnn import FrequencyRNNBlock, TimeRNNBlock
        ...
        **unused_kwargs
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                if all_resolutions[i_level] in rnn_resolutions:
                    modules.append(FrequencyRNNBlock(in_ch, 128, in_ch))
                    modules.append(TimeRNNBlock(in_ch, 128, in_ch))
                hs_c.append(in_ch)
...
#         modules.append(AttnBlock(channels=in_ch))
        time_encoder_layer = nn.TransformerEncoderLayer(d_model=in_ch, nhead=8)
        modules.append(nn.TransformerEncoder(time_encoder_layer, num_layers=4))
        freq_encoder_layer = nn.TransformerEncoderLayer(d_model=in_ch, nhead=8)
        modules.append(nn.TransformerEncoder(freq_encoder_layer, num_layers=4))
    ...
    def forward(self, clean, degrad, time_cond):
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                if h.shape[-1] in self.attn_resolutions: # edit: check H dim (-2) not W dim (-1)
                    h = modules[m_idx](h)
                    m_idx += 1

                if h.shape[-1] in self.rnn_resolutions:
                    h, skip = modules[m_idx](h)
                    m_idx += 1
                    h = modules[m_idx](h, skip)
                    m_idx += 1
                hs.append(h)
        ...
        h = rearrange(h, 'b c t f -> (b f) t c', c=c, t=t, f=f) 
        h = modules[m_idx](h)  # Freq TF block
        m_idx += 1
        
        h = rearrange(h, '(b f) t c -> (b t) f c', c=c, t=t, f=f)
        h = modules[m_idx](h) # Time TF block
        m_idx += 1

        h = rearrange(h, '(b t) f c -> b c t f', c=c, t=t, f=f)
        h = modules[m_idx](h, temb)  # ResNet block
        m_idx += 1
        ...
        score = rearrange(h, 'b c t f -> b f t c')
        return score
