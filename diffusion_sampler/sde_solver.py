...
class Heun2ndSampler(nn.Module):
    ...
    def forward(self, shape, operator=None, y=None, c_embedding=None, t_embedding=None):
        requires_grad = True if operator is not None else False
        x_next = self.timesteps[0] * torch.randn(shape,
        ...
        for i, (t, t_next) in iterator:
            ...
            # Euler step
            d, d_cond = self.estimate_dx_dt(x_hat, t_hat, shape, operator, y, t_embedding=t_embedding, c_embedding=c_embedding)
            if self.print_grad_norm : print(torch.linalg.norm(d))
            x_next = x_hat + (t_next - t_hat) * d

            # Guidance Term
            if d_cond is not None:
                if self.print_grad_norm : print(torch.linalg.norm(d_cond))
                x_next = x_next + (t_next - t_hat) * d_cond 

            # 2nd order correction
            if i < self.steps - 1:
                d_prime, d_cond_prime = self.estimate_dx_dt(x_next, t_next, shape, operator, y=y, t_embedding=t_embedding, c_embedding=c_embedding)
                x_next = x_hat + (t_next - t_hat) * 0.5 * (d + d_prime)
                if d_cond_prime is not None:
                    x_next = x_next + (t_next - t_hat) * 0.5 * (d_cond + d_cond_prime)
        return x_next

    '''
    Diffusion Posterior Sampling
    '''
    def estimate_dx_dt(self, x_t, t, shape, operator=None, y=None, t_embedding=None, c_embedding=None):
        # Unconditional Score
        t_ones = t * torch.ones(shape[0], device=self.device, requires_grad=False)
        x_t = x_t.detach()
        if operator is not None and y is not None: x_t.requires_grad_(True)
        denoised = self.sde.denoiser(x_t, t_ones, y, t_embedding=t_embedding, c_embedding=c_embedding)
        
        # Score of the posterior
        d_cond = None
        if operator is not None and y is not None:
#             y = y.detach()
            unet_input = torch.cat([denoised, y], dim=-1)
            pred = operator(unet_input)
            loss_mode = 'l2'
            loss = self.loss_fn(y, pred, loss_mode=loss_mode)
#             difference = y - operator(unet_input)
#             loss = torch.sum(torch.square(difference))
            loss.backward()
            weight = self.calc_weight(loss, loss_mode)
            gradient = x_t.grad
            d_cond = weight * gradient
#             print('dcond',torch.linalg.norm(d_cond))

        d_uncond = (x_t - denoised) / t
        d_uncond = d_uncond.detach()
#         print('duncond',torch.linalg.norm(d_uncond))
